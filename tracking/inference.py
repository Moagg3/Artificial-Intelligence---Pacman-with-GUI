...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for x in self.allPositions:
            ...
            mulFactor = self.beliefs[x]

            self.beliefs[x] = self.getObservationProb(observation ,pos ,x, jailpos) * mulFactor
    ...
    def elapseTime(self, gameState):
        ...
        for x in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, x)

            mulFactor2 = self.beliefs[x]

            for y in newPosDist.keys():
                check[y] = (newPosDist[y] * mulFactor2) + check[y]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for z in self.particles:
            factor = (self.getObservationProb(observation, Pos, z, jail))
            dist[z] = factor + dist[z]

        for r in val:
            if r!=0:
                store = False

        tempStore =  self.initializeUniformly(gameState)       
    ...
    def elapseTime(self, gameState):
        ...
        for x, y in enumerate(particle):
            temp = self.getPositionDistribution(gameState, y).sample()
    ...
    def getBeliefDistribution(self):
        ...
        storeTemp = util.Counter()

        particle = self.particles

        for x in particle:
            storeTemp[x] = 1 + storeTemp[x]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        ghost = self.numGhosts
        legPos = self.legalPositions
 
        self.particles = list(itertools.product(legPos, repeat = ghost))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for c in part:
            ...
            for i in range(ghost):
                add = self.getObservationProb(observation[i], pacPos, c[i], self.getJailPosition(i))
                factor = add * factor
            dist[c] += factor

        for g in dist.values():
            if g!=0:
                store = False

        start = self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for itr in enumerate(newParticle):

                ghost = self.ghostAgents[itr[0]]
                temp = itr[0]

                newParticle[itr[0]] = (self.getPositionDistribution(gameState, oldParticle, temp, ghost)).sample()
