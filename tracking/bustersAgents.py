...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        gTemp = []
        for s in livingGhostPositionDistributions:
            gTemp.append(s.argMax())

        temp = []
        for g in gTemp:
            store = self.distancer.getDistance(g,pacmanPosition)
            temp.append(store)
        

        work = gTemp[temp.index(min(temp))]
        temp2 = []
        for h in legal:
            store2 = self.distancer.getDistance(work,Actions.getSuccessor(pacmanPosition, h))
            temp2.append(store2)

        return legal[temp2.index(min(temp2))]
